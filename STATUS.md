...
| 4 | ~~**TQ-O3 (lupa sandi)**~~ — **LULUS 2026-08-12**: alur reset end-to-end terbukti dengan pengguna nyata (generate link via admin API → verify → "Buat Sandi Baru" → sandi baru → masuk). Catatan: email publik masih tertahan rate-limit; jalur admin (generate_link) bebas antrean | ~~2026-08-12~~ | ~~Email baru terkirim~~ |
...
- 2026-08-12 — **TQ-O3 LULUS (uji nyata)**: alur lupa sandi end-to-end terbukti — generate_link via service_role (tanpa email, bebas rate-limit) → klik → verify → redirect `png.lazisnu.site#access_token=…&type=recovery` → form "Buat Sandi Baru" → `PUT /auth/v1/user` → "Sandi baru tersimpan" → login sukses. **Pitfall:** tautan yang ditampilkan di chat ikut "diklik" pratinjau otomatis (OTP sekali pakai → otp_expired) — tautan sensitif diserahkan lewat berkas lokal (`D:\bukti-builder\tautan-reset.txt`). Sandi admin 1 dirotasi (berkas kredensial lokal diperbarui). service_role kini tercatat di PRIBADI-JANGAN-DIUNGGAH.md (lokal saja) + salinan `D:\bukti-builder\service-role.txt`.
